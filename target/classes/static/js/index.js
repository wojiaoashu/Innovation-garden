...
function dealSearchList(){
    ...
    // function originalBrotherSearchItemsClassName(clickedItem){
    ...
    // 关键字导航清空样式
    function originalSearchItemsClassName(){
    ...
    searchIcon.onclick = function(){
        ...
        getPage('searchByInputKey',searchInput.value.trim());
...
// 按默认或搜索关键字来获取文章列表展示，type的值all代表全部文章，search代表搜索出来的文章。key的值"所有"是指搜索当中的所有。
function getPage(type,key){
    ...
    }else if(type == 'search'){ // 按搜索导航关键字搜索的情况
    ...
    }else if(type == 'searchByInputKey'){ //按输入搜索的情况
        myAjax(homePath + 'main/pageCountByInputKey?pageKey='+key,'get','',function(ajax_res){
            if(ajax_res.code == 500){
                alert(ajax_res.errorMessage);
            }else if(ajax_res.code == 200){
                // console.log("按输入检索，查到了几个："+ajax_res.data);
                lastIndex = Math.floor(ajax_res.data/8) + (ajax_res.data%8 ? 1:0);
            }
            // 展示分页页码 和 列表
            changePage(lastIndex,type,key);
...
function changePage(lastIndex,type,key){
    ...
    var pageMoreFirst = pages[0];
    var pageMoreLast = pages[4];
    
    // 搜索、首次加载、搜索 都会重新生成页码。避免切换后影响后者，得先清空/恢复样式
    ...
    for(let i=1; i<pages.length-1; i++){
        pages[i].onclick = function(){
            ...
            }else if(type == 'searchByInputKey'){
                myAjax(homePath + 'main/get_OnePageArticlesByInputKey?pageIndex='+pageValue+'&&pageKey='+key,'get','',function(ajax_res){
                    if(ajax_res.code == 500){
                        alert(ajax_res.errorMessage);
                    }else{
                        showArticlesList(ajax_res.data);
            ...
            loginSubmit.onclick = function(){
                ...
                myAjax( homePath+'auth/login','post',json_str,function(ajax_res){
                    if(ajax_res.code == 500){
                        ...
                        // 记录令牌
                        sc.setItem("loginId",ajax_res.data.loginId);
                        sc.setItem("token",ajax_res.data.token);
                        sc.setItem( "createTime",JSON.stringify( new Date().getTime() ) );
                        sc.setItem("showName",showName);
                        // 重新开始检查登录是否超时
                        checkExp();
                    }
                });
        ...
        articleTitles[i].onclick = articlePres[i].onclick = function(){
            ...
            myAjax(homePath + 'main/get_article?aid='+ aid,'get','',function(ajax_res){
                if(ajax_res.code == 200){
                    ...
                    let o_dt = new Date( a.articleDatetime.split('.')[0].replace(/-/g, "/").replace(/T/g, " ") + ' GMT+0000 (CST)' );
...
function showEditor(){
    ...
    editorPreview.addEventListener('click', function () {
        ...
        if(html.length > 400000){
            let toLess = (html.length - 400000)/1024;
            if(toLess >= 1){
                toLess = parseInt(toLess);
            }
            alert("文章过长，请减少文字或缩减图片后再预览！大约需要减少"+toLess+"KB");
            return;
        }
    ...
    editorSubmit.addEventListener('click', function () {
        ...
        if(publish){
            ...
            if(html.length > 400000){
                ...
                alert("文章过长，请减少文字或缩减图片后再提交！大约需要减少"+toLess+"KB");
                return;
            }

            // let filterHtml = filterXSS(html)  // 此处进行 xss 攻击过滤，会使base64保存的图片被过滤掉。
            let json_con = {"authorId":sc.getItem("loginId"),"articleTitle":editorTitle_con,"articleBody":html};
            let json_str=JSON.stringify(json_con);
            // editor.txt.clear();
            // editorTitle.value = '';
...
function isExpByLocal(t){
    let et = t?t:iTime; //120分钟

    if(sc.getItem("createTime") == null){
        return true;
    }
    if(new Date().getTime() - sc.getItem("createTime") < et){
        return false;
    }else{
        return true;
    }
...
function myAjax(url,type,json_con,callback){
    ...
    ajax.setRequestHeader("loginId",sc.getItem("loginId"));
    ajax.setRequestHeader("token",sc.getItem("token"));
...
// safari无痕模式禁用localStorage问题，要实现cookie的setItem、getItem、clear功能
function localStorageOrCookie(){
    ...
    cookies.prototype = {
        ...
        setItem:function(c_name, value){
            var exp = new Date();
            exp.setTime(exp.getTime() + iTime);
        ...
        clear:function(){
            var coki = new cookies();
            var kys = document.cookie.split(';');
            var expt = new Date();
            expt.setTime(expt.getTime() - 1);

            for(let i = 0; i<kys.length; i++){

                let k = kys[i].split('=')[0];
                let v = coki.getItem(k);
                document.cookie= k + "="+v+";expires="+expt.toGMTString();
            }
        }
    }

    // 若浏览器不支持localstorage就改用cookie
    try{
        sc = window.localStorage;
    }catch(e){
        window.isPrivateMode = true;
    }
    // 不是 Safari 无痕模式并且能用 localStorage，结果：ff本来不设置也可以用，safari设置了也用不了localstorage
...
window.onload = function(){
    ...
    homePath = 'http://127.0.0.1:8080/'; //47.107.148.70
    checkLoginTimer = null;
    isLogin = true;
    sc = null; // 本地存储块
    iTime = 7200000; // 保鲜时长120分钟
    var pn = location.pathname.split('/').pop().replace(/.html/g,'');

    // 兼容Safari禁用localstorage
    localStorageOrCookie();

    // 检查登录超时
    if(!isExpByLocal()){
        changeLogShow('logout',sc.getItem("showName"));
    }else{
        changeLogShow('login','');
        sc.clear();
    }
